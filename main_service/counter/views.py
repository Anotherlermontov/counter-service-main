...
def increment(request: Request) -> JsonResponse:
    form = IncrementForm(request.GET)
    ...
    try:
        tx_params = get_increment_tx_params(
            counter_contract_address=settings.COUNTER_ADDRESS,
            from_address=wallet.address,
            amount=int(amount),
        )
    except Exception as exc:
        return JsonResponse({'error': f'Tx Builder request failed: {exc}'}, status=500)

    try:
        chain_handler = ChainHandler()
        tx_hash = chain_handler.send_increment_transaction(
            private_key=wallet.private_key, tx_params=tx_params
        )
    except Exception as exc:
